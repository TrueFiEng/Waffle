...
const createMockProvider = (ganacheOptions: GanacheOpts = {}): Provider => {
  const options = {...defaultGanacheOptions, ...ganacheOptions};
  return new providers.Web3Provider(ganache.provider(options));
};

const getWallets = async (provider: Provider): Promise<Wallet[]> =>
  accounts.map((account) => new Wallet(account.secretKey, provider));

const withENS = (provider: any, ensAddress: string): Provider => {
  const chainOptions = {name: 'providerWithENS', ensAddress, chainId: 0};
  return new providers.Web3Provider(provider._web3Provider, chainOptions);
};

export {createMockProvider, getWallets, defaultGanacheOptions, withENS};
