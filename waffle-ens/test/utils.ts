...
export const getWallets = (): Wallet[] => {
  ...
  const provider = new providers.Web3Provider(Ganache.provider({accounts: defaultAccounts}) as any);
  return defaultAccounts.map((x: any) => new Wallet(x.secretKey, provider));
};
