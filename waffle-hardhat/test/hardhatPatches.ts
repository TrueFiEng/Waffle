...
const callHistory: { address: string, data: string }[] = []

const init = (waffle.provider as any)._hardhatNetwork.provider._wrapped._wrapped._wrapped._init

function patchCallHistory() {
  (waffle.provider as any)._hardhatNetwork.provider._wrapped._wrapped._wrapped._node._vmTracer._vm.on('beforeMessage', (message: any) => {
    if (message.to) {
      callHistory.push({
